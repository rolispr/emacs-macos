...
class MacmodTTS: NSObject {
    static let shared = MacmodTTS()
    ...
    func speakOpenAI(text: String, apiKey: String, voice: String) {
        ...
        speechTask = Task {
            ...
            for chunk in chunks {
                ...
                let data = try await MacmodOpenAI.generateSpeech(from: chunk, voice: voice, apiKey: apiKey)
...
extension MacmodTTS: AVSpeechSynthesizerDelegate {
    func speechSynthesizer(_ synthesizer: AVSpeechSynthesizer, didStart utterance: AVSpeechUtterance) {
        print("[MacmodTTS] Started speaking")
    }

    func speechSynthesizer(_ synthesizer: AVSpeechSynthesizer, didFinish utterance: AVSpeechUtterance) {
        print("[MacmodTTS] Finished speaking")
    }

    func speechSynthesizer(_ synthesizer: AVSpeechSynthesizer, didCancel utterance: AVSpeechUtterance) {
        print("[MacmodTTS] Cancelled speaking")
    }
}

extension MacmodTTS: AVAudioPlayerDelegate {
    @MainActor
    ...
    static func generateSpeech(from text: String, voice: String = "alloy", apiKey: String) async throws -> Data {
        ...
        if let httpResponse = response as? HTTPURLResponse,
           httpResponse.statusCode != 200 {
            throw NSError(domain: "MacmodOpenAI", code: httpResponse.statusCode, userInfo: [NSLocalizedDescriptionKey: "HTTP \(httpResponse.statusCode)"])
        }
